 ...
 */
@Slf4j
public class AiExpressionRecServiceImpl implements IAiExpressionRecService {
    //设备类型
    ...
    public R<DetectionResponse> detect(String imagePath){
        try (ExpressionModel model = getExpressionModel()){
            R<DetectionResponse> result = model.detect(imagePath);
            if(result.isSuccess()){
    ...
    public R<List<ExpressionResult>> detect(String imagePath, DetectionResponse faceDetectionResponse) {
        try (FaceDetModel faceDetModel = getFaceDetModel();
            ...
            if(detResult.isSuccess()){
                ...
                return R.fail(-1,"识别失败: " + detResult.getMessage());
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
     ...
     */
    @Override
    public R<ExpressionResult> detect(String imagePath, DetectionRectangle faceDetectionRectangle) {
        try (FaceDetModel faceDetModel = getFaceDetModel();
             ExpressionModel model = getExpressionModel()){
            BufferedImage image = ImageIO.read(new File(Paths.get(imagePath).toAbsolutePath().toString()));
            R<DetectionResponse> detResult = faceDetModel.detect(image);
            if(detResult.isSuccess()){
                ...
                return R.ok(detResult.getData().getDetectionInfoList().get(0).getFaceInfo().getExpressionResult());
            }else{
                log.info("人脸检测失败：{}", detResult.getMessage());
                return R.fail(-1,"识别失败: " + detResult.getMessage());
            }
